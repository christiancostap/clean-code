...
describe('Login Router', () => {
  ...
  test('Should return 500 if httpRequest provided has no body', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.route({})
    expect(httpResponse.statusCode).toBe(500)
    expect(httpResponse.body).toEqual(new ServerError())
  ...
  test('Should call AuthUseCaseSpy with correct parameters', async () => {
  ...
  test('Should return 500 if EmailValidator throws error.', async () => {
    const useCaseSpy = makeAuthUseCase()
    const emailValidatorSpy = makeEmailValidatorWithError()
    const sut = new LoginRouter(useCaseSpy, emailValidatorSpy)
    const httpRequest = {
      body: {
        email: 'any_email@mail.com',
        password: 'any_password'
